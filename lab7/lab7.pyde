...
class Tree:
  ...
  def draw(self):
    self._helperD(self.root(),int(width/2),0)
  
  def _helperD(self,p,x,y):
    global xdistance
    one = TextBox(str(p._node._element),x,y)
    # one.setLocation(x-int(one.width()/2),y)
    one.draw()
    if self.left(p)!=None:
        left = TextBox(str(self.left(p).element()),one._x-xdistance,one._y+ydistance)
        one.drawLineToOtherBoxBelow(left)
        # left.draw()
        self._helperD(self.left(p),one._x-xdistance,one._y+ydistance)
    if self.right(p)!=None:
        right = TextBox(str(self.right(p).element()),one._x+xdistance,one._y+ydistance)
        one.drawLineToOtherBoxBelow(right)
        # right.draw()
        self._helperD(self.right(p),one._x+xdistance,one._y+ydistance)
    xdistance-=30
...
class TextBox:
    ...
    def __init__(self, text, x=0, y=0):
        ...
        # print("initial width: " + str(self.width()))
...
def btHelper(s,n):
    ...
    if s==n:
        ...
        # m=int(math.ceil((n+s)/2))
        m=math.ceil((n+s)/2)
        print(s,n)
        print((s+n)/2)
        t._add_root(m)
        t._attach(t.root(),btHelper(s,m-1),btHelper(m+1,n))
        return t
def makeTree(i):
    t = Tree()
    t._add_root(i[1])
    t._attach(t.root(),mtHelper(i[0]),mtHelper(i[2]))
    return t
def mtHelper(i):
    t = Tree()
